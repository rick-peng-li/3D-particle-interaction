...
export function useShapeGenerator() {
  ...
  const generateTorus = (radius = 1.5, tube = 0.6) => {
    const positions = new Float32Array(PARTICLE_COUNT * 3)
    for (let i = 0; i < PARTICLE_COUNT; i++) {
  ...
  const generateHeart = (scale = 0.15) => {
    const positions = new Float32Array(PARTICLE_COUNT * 3)
    for (let i = 0; i < PARTICLE_COUNT; i++) {
  ...
  const generateStar = (outerRadius = 2.5) => {
    return generateFromCanvasShape('★', outerRadius * 20)
  }

  const generateText = (text, size = 10) => {
    return generateFromCanvasShape(text, 60, 2)
  }

  const generateFromCanvasShape = (text, fontSize, densityFactor = 1) => {
