export const ShapeGenerator = {
  ...
  getRandomSpherePoint(radius: number) {
    const u = Math.random();
    const v = Math.random();
  ...
  generateSphere(radius = 2): Float32Array {
  ...
  generateTorus(radius = 1.5, tube = 0.6): Float32Array {
    const positions = new Float32Array(this.count * 3);
    for (let i = 0; i < this.count; i++) {
  ...
  generateHeart(scale = 0.15): Float32Array {
    const positions = new Float32Array(this.count * 3);
    for (let i = 0; i < this.count; i++) {
  ...
  generateStar(outerRadius = 2.5): Float32Array {
    return this.generateFromCanvasShape('★', outerRadius * 20);
  },

  generateText(text: string): Float32Array {
    return this.generateFromCanvasShape(text, 60);
  },

  generateFromCanvasShape(text: string, fontSize: number): Float32Array {
