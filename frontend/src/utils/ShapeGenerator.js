...
function generateSphere(radius = 2) {
...
function generateTorus(radius = 1.5, tube = 0.6) {
    const positions = new Float32Array(count * 3);
    for (let i = 0; i < count; i++) {
...
function generateHeart(scale = 0.15) {
    const positions = new Float32Array(count * 3);
    for (let i = 0; i < count; i++) {
...
function generateStar(outerRadius = 2.5, innerRadius = 1.0, thickness = 0.5) {
    return generateFromCanvasShape('★', outerRadius * 20);
}

function generateText(text, size = 10) {
    return generateFromCanvasShape(text, 60, 2);
}

function generateFromCanvasShape(text, fontSize, densityFactor = 1) {
